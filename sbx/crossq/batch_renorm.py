...
class BatchRenorm(Module):
  ...
  If we define a model with BatchRenorm, for example::
    ...
    BRN = nn.BatchRenorm(use_running_average=False, momentum=0.9, epsilon=1e-5, dtype=jnp.float32)
  ...
  def __call__(self, x, use_running_average: Optional[bool] = None):
    ...
    if use_running_average:
      ...
      if not self.is_initializing():
        ...
        custom_var = is_warmed_up * tmp_var + (1. - is_warmed_up) * custom_var
        custom_mean = is_warmed_up * tmp_mean + (1. - is_warmed_up) * custom_mean
